...
Button::Button(Vector2D position, SDL_Color color, double width,
               double height, std::string text)
    : Object(position, NULL), color_(color), width_(width), height_(height), text_(text)
{}
;
...
void Button::draw(SDL_Renderer *renderer) const {
    ...
    SDL_Rect destButton = {(int)upper_left_coords.x_,
    ...
    SDL_Rect textRect = { (int)(upper_left_coords.x_ + (width_ - surface->w) / 2),
        (int)(upper_left_coords.y_ + (height_ - surface->h) / 2), width_, height_ };
